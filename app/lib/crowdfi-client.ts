...
export { CLUSTER_ENDPOINTS, CROWDFI_IDL, ClusterKind, WalletKey, WalletLike, getAvailableWallets, getWalletByKey };
...
export async function connectWallet(walletKey: WalletKey) {
  ...
  if (!wallet) throw new Error(`${walletKey === "phantom" ? "Phantom" : "Solflare"} wallet not found`);
...
export async function fetchCampaigns(
  ...
  const rows = await (program.account as { campaign: { all: () => Promise<Array<{ publicKey: PublicKey; account: unknown }>> } }).campaign.all();
...
export function deriveCampaignPda(owner: PublicKey, campaignId: bigint, programId?: string | PublicKey) {
  ...
  return PublicKey.findProgramAddressSync([Buffer.from("campaign"), owner.toBuffer(), idSeed], pid)[0];
}

export function deriveVaultPda(campaign: PublicKey, programId?: string | PublicKey) {
  const pid = resolveProgramId(programId);
  return PublicKey.findProgramAddressSync([Buffer.from("vault"), campaign.toBuffer()], pid)[0];
}

export function deriveDonationPda(campaign: PublicKey, donor: PublicKey, programId?: string | PublicKey) {
  const pid = resolveProgramId(programId);
  return PublicKey.findProgramAddressSync([Buffer.from("donation"), campaign.toBuffer(), donor.toBuffer()], pid)[0];
}
...
export async function sendCreateCampaignTx(
  ...
  const campaign = deriveCampaignPda(connected.publicKey, input.campaignId, programId);
...
export async function sendDonateTx(
  ...
  const connected = getConnectedWallet(wallet);
  const program = createProgram({ cluster, programId, wallet: connected });
  const campaign = deriveCampaignPda(input.campaignOwner, input.campaignId, programId);
  const vault = deriveVaultPda(campaign, programId);
  const donation = deriveDonationPda(campaign, connected.publicKey, programId);

  return program.methods
    .donate(new BN(input.campaignId.toString()), new BN(input.amountLamports.toString()))
    .accounts({
